...
DIR_CELLS_LIST=""
NUM_CORES=16
PLOIDY_INTERVALS=""                       # add to the defaults
...
do
    case "${1}" in
        ...
        --ploidy        ) PLOIDY_INTERVALS="$2"                                 ; shift; ;;
        --cells         ) DIR_CELLS_LIST="$2"                                   ; shift; ;;
        --ncores        ) NUM_CORES="$2"                                        ; shift; ;;
...
printf '' > ${DIR_CELLS_LIST}_binUnsorted.sh
...
if [ -z "${NB_BINS}" ]; then
    echo "${DIR_GENOME}/${BINNING}" does not exist"!"
    exit 1
fi
...
do
    ...
    # Bin reads
    echo ${DIR_SCRIPTS}/binUnsorted ${DIR_GENOME}/${BINNING} ${NB_BINS} "<(${Z}cat ${file})" `echo ${file} | awk -F ".bed" '{print $1}'` ${file}_mapped >> ${DIR_CELLS_LIST}_binUnsorted.sh
done < ${DIR_CELLS_LIST}
cat ${DIR_CELLS_LIST}_binUnsorted.sh | parallel -j $NUM_CORES
...
${DIR_SCRIPTS}/process_parallel.R ${DIR_GENOME} ${DIR_INPUT} ${statFile} data ${SEGMENTATION} ${BINNING} ${CLUSTERING_LINKAGE} ${CLUSTERING_DISTANCE} ${COLOR} ${SEGMENTATION_REF}_mapped ${FACS} ${FILE_FACS} $( [[ $MASK_SEXCHRS == 1 ]] && echo 0 || echo 1 ) ${MASK_BADBINS} ${NUM_CORES} "${PLOIDY_INTERVALS}"
...
tar -czf "${DIR_INPUT%/}_archive.tar.gz" --exclude '*.bed' --exclude '*.bed.gz' --exclude '*.tar.gz' -C "${DIR_INPUT}" .
